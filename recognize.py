...
from char_crop import crop_digit
...
class App:
    def __init__(self):
        ...
        # 创建标签和下拉菜单小部件
        self.label_map = ttk.Label(self.start_window, text="选择模型")
        ...
        self.button2 = ttk.Button(self.start_window, text="确定", command=self.get_map)
        self.button2.place(relx=0.80, rely=0.7, anchor=CENTER, width=50, height=35)
    ...
    def recognize(self):
        ...
        if self.mode == 'draw':
            ...
            for i in range(1, num_row):
                self.canvas.create_line(0, 200 * i, 1240, 200 * i, dash=(5, 5), tag='guide')
        img = np.array(img)
        cropped_images = crop_digit(img)
        global digits
        if len(cropped_images) == 0:
            self.printf('没有识别到数字')
        else:
            for crop_lines in cropped_images:
                for img in crop_lines:
